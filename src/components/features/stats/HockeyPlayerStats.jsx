...
export default function HockeyPlayerStats({ stats }) {
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg shadow-lg p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-emerald-100">🏑 Hockey Player</p>
          </div>
        </div>
      </div>

      {/* Quick Stats — hockeyMatchesPlayed */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <QuickStat
          icon={<Trophy className="w-8 h-8 text-emerald-500" />}
          value={stats.hockeyMatchesPlayed ?? stats.matchesPlayed ?? 0}
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6 flex flex-wrap gap-8">
          ...
          {stats.greenCards !== undefined && (
            <StatItem label="🟩 Greens" value={stats.greenCards} />
          )}
          {stats.penaltyCorners !== undefined && (
            <StatItem label="🎯 PC" value={stats.penaltyCorners} />
          )}
